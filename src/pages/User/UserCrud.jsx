...
const UserCrud = () => {
  ...
  const {
    ...
    queryKey: [`https://auth.betaja.ir/api/user/id?id=${id}`],
    queryFn: getApi,
    select: (res) => res.data,
  ...
  const {
    ...
    queryKey: [`https://auth.betaja.ir/api/organizations/root-organization`],
    queryFn: getApi,
    select: (res) => res.data,
    enabled: !!id,
  });
  ...
  const onSubmitHandler = () => {
    ...
    mutate({
      ...
      endpoint: 'https://auth.betaja.ir/api/user',
      data: {
        ...DTO,
        ...(pageType === 'EDIT' ? {id} : {})
      }
